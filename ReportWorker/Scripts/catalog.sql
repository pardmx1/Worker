WITH Datos AS ( 
    ...
    WHERE f.active = 1 and fm.Name not like 'FindStageOnReject_%'
), 

Catalogo AS ( 
    SELECT *, ROW_NUMBER() OVER (ORDER BY GroupName) AS [No.] FROM(
    SELECT DISTINCT TOP 100 PERCENT GroupName, Variable, CONCAT([GroupName], '_', substring(Variable,1,100)) AS Alias
